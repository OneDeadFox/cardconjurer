...
(function(){
 ...
 function insert(frame,kind,cropped){if(!frame||!groups[kind])return;sync();const before=createDesignStateSnapshot(),id=ensureDesignLayerId(frame),from=cropped?regions[kind]:{x:0,y:0,width:1,height:1},to=transform(frame),fields={};
  groups[kind].forEach(role=>{let key=Object.keys(card.text||{}).find(k=>card.text[k].frameAnchor?.id===id&&card.text[k].standardRole===role);if(!key)key=!card.text[role]?.frameAnchor?role:role+'-'+id;const old=card.text[key];let field=clone(templates[role]);field.text=old?.text||'';field.standardRole=role;field.customField=true;remap(field,from,to,false);attach(field,frame);fields[key]=field;});loadTextOptions(fields,false);
  if(kind==='type'){card.setSymbolBounds={x:.9213,y:.591,width:.12,height:.041,vertical:'center',horizontal:'right'};remap(card.setSymbolBounds,from,to,true);attach(card.setSymbolBounds,frame);resetSetSymbol();}
 ...
 function mount(container,frame){let panel=container.querySelector('.frame-standard-fields');if(!panel){panel=document.createElement('div');panel.className='frame-standard-fields wide';panel.innerHTML='<h3>Add standard fields</h3><label>Preset<select class="input preset-kind"><option value="title">Title and mana cost</option><option value="type">Type line and set symbol</option><option value="rules">Rules and flavor text</option><option value="pt">Power / toughness</option></select></label><label>Artwork layout<select class="input preset-layout"><option value="full">Full-card artwork (regular frames)</option><option value="cropped">Cropped component</option></select></label><p>Uses standard placement relative to this layer. Existing standard text is preserved. The type preset positions the current rarity-colored set symbol.</p><button type="button" class="input preset-add">Insert fields</button>';container.appendChild(panel);}
