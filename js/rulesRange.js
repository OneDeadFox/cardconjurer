(function () {
	...
	function syncElements(){
		...
		ranges().forEach(function(range){var uniformLanes=new Map(),lanes=[];moduleLayouts(range).forEach(function(layout){layout.module.elements.forEach(function(entry){var bounds=elementBounds(entry);if(!bounds)return;if(entry.offset){applyOffset(bounds,layout.bounds,entry.offset);if(Number.isFinite(entry.fitHeight))bounds.height=Math.max(1,layout.bounds.height*card.height-entry.offset.y-entry.fitHeight)/card.height;}else if(entry.relative)applyRelative(bounds,layout.bounds,entry.relative);else return;if(entry.kind==='text')changedText=true;else changedFrames=true;});var groups=new Map();layout.module.elements.forEach(function(entry){if(entry.kind!=='text'||!entry.collisionGroup)return;if(!groups.has(entry.collisionGroup))groups.set(entry.collisionGroup,[]);groups.get(entry.collisionGroup).push(entry);});groups.forEach(function(entries){var cost=entries.find(function(item){return item.collisionRole==='cost';}),name=entries.find(function(item){return item.collisionRole==='name';});if(cost&&name)lanes.push({cost:cost,name:name});});});lanes.forEach(function(lane){var left=elementTarget(lane.cost),right=elementTarget(lane.name);if(!left||!right)return;var x=Math.min(lane.cost.offset.x,lane.name.offset.x),end=Math.max(lane.cost.offset.x+lane.cost.offset.width,lane.name.offset.x+lane.name.offset.width),total=end-x,gap=Math.max(5,card.width*.004),desired=measureSingleLine(left,0)+gap,share=Math.min(total*.65,Math.max(total*.35,desired)),base=Math.min(left.x,right.x);left.x=base;left.width=Math.max(1,share-gap)/card.width;right.x=base+share/card.width;right.width=Math.max(1,total-share)/card.width;[[left,share-gap,'cost'],[right,total-share,'name']].forEach(function(part){var field=part[0],available=part[1],reduction=Number(field.rangeFontReduction)||0;field.clipToBounds=true;while(reduction<25&&measureSingleLine(field,reduction)>available)reduction++;field.rangeFontReduction=reduction;if(range.uniformTextSize)uniformLanes.set(part[2],Math.max(uniformLanes.get(part[2])||0,reduction));});});if(range.uniformTextSize)lanes.forEach(function(lane){[['cost',lane.cost],['name',lane.name]].forEach(function(pair){var field=elementTarget(pair[1]);if(field)field.rangeFontReduction=uniformLanes.get(pair[0])||0;});});});
