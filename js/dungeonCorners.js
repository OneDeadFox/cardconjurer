...
(function () {
 'use strict';
 var teeArms={'t-up':['left','right','up'],'t-down':['left','right','down'],'t-left':['up','down','left'],'t-right':['up','down','right']};
 ...
 function collect(segments){
  ...
  });n.radius=Math.max(0,Math.min((Number(n.settings.size)||.02)*card.height,...Object.values(n.arms).map(function(v){return v*.45;})));
   n.connectedArms=Object.keys(n.arms);
   if(teeArms[n.settings.style])teeArms[n.settings.style].forEach(function(arm){if(!n.arms[arm])n.arms[arm]=n.radius;});
  });return nodes;
 }
 ...
 function geometry(segments){
  ...
  nodes.forEach(function(n){if(!custom(n))return;if(teeArms[n.settings.style]){Object.keys(n.arms).forEach(function(arm){var d=directions[arm];edges.push([[n.x+d[0]*n.radius,n.y+d[1]*n.radius],[n.x,n.y]]);});return;}var arms=Object.keys(n.arms);for(var i=0;i<arms.length;i++)for(var j=i+1;j<arms.length;j++){var a=directions[arms[i]],b=directions[arms[j]];if(a[0]*b[0]+a[1]*b[1]===0)edges.push(curved(n,a,b));}});
 ...
 function mount(){if(dialog)return;dialog=document.createElement('dialog');dialog.className='dungeon-corner-editor';dialog.innerHTML='<button type="button" id="corner-close" aria-label="Close corner editor">×</button><h3>Wall corner</h3><p>Choose a shape. Shared corners update together. T choices keep existing walls connected.</p><div class="corner-presets"></div><label>Corner size (pixels)<input id="corner-size" type="number" min="1" max="1000"></label><h4>Transition</h4><label>Wall end<select id="corner-arm"></select></label><label><input id="corner-fade" type="checkbox"> Fade this end into the background</label><label>Fade length (pixels)<input id="corner-length" type="number" min="1" max="10000"></label><p>The selected arm fades outward from the corner toward the next junction or doorway.</p><button type="button" id="corner-reset">Reset corner</button> <button type="button" id="corner-tab">Go to Dungeon tab</button>';
  [['square','Square','M10 44V10H44'],['rounded','Rounded','M10 44V30Q10 10 30 10H44'],['beveled','Beveled','M10 44V30L30 10H44'],['inward','Inward cut','M10 44V30H30V10H44'],['t-up','T up','M7 35H47M27 35V7'],['t-down','T down','M7 19H47M27 19V47'],['t-left','T left','M35 7V47M35 27H7'],['t-right','T right','M19 7V47M19 27H47']].forEach(function(s){var b=document.createElement('button');b.type='button';b.dataset.style=s[0];b.innerHTML='<svg viewBox="0 0 54 54" aria-hidden="true"><path d="'+s[2]+'" fill="none" stroke="currentColor" stroke-width="6"/></svg>'+s[1];b.onclick=function(){var n=resolve();if(n)setSettings(n,{style:s[0]});};dialog.querySelector('.corner-presets').appendChild(b);});
