(function () {
	...
	function applyEditor() {
		...
		var name=document.querySelector('#canvas-element-name').value.trim(); var rotation=val('rotation'); if(range){range.name=name||range.name;range.direction=document.querySelector('#canvas-element-flow').value;range.rotation=rotation;range.autoSizeModules=!!document.querySelector('#canvas-element-auto-size').checked;range.uniformTextSize=!!document.querySelector('#canvas-element-uniform-text').checked;RulesRange.refresh();}
		...
		if(editorArea.kind==='text'&&window.RulesTextStyles){var read=function(id){return document.querySelector('#canvas-style-'+id);};RulesTextStyles.update(editorArea.key,{font:read('font').value,fontSize:Number(read('font-size').value)||0,size:Math.max(.001,Number(read('size').value)||.038),align:read('align').value,color:read('color').value,lineSpacing:Number(read('line-spacing').value)||0,kerning:Number(read('kerning').value)||0,shadowX:Number(read('shadow-x').value)||0,shadowY:Number(read('shadow-y').value)||0,outlineWidth:Number(read('outline').value)||0,oneLine:read('one-line').checked,noVerticalCenter:read('top').checked},false);}
		drawTextBuffer(); drawFrames(); drawCard();
	}
	function refreshTextStyle(key){if(!window.RulesTextStyles)return;var field=card.text[key],list=document.querySelector('#canvas-style-font');if(!field||!list)return;if(!list.options.length)list.innerHTML=RulesTextStyles.fontOptions();if(field.font&&!RulesTextStyles.fonts.includes(field.font)){var option=document.createElement('option');option.value=field.font;option.textContent=field.font;list.appendChild(option);}list.value=field.font||'mplantin';[['font-size',field.fontSize||0],['size',field.size||.038],['align',field.align||'left'],['color',/^#[\da-f]{6}$/i.test(field.color||'')?field.color:'#000000'],['line-spacing',field.lineSpacing||0],['kerning',field.kerning||0],['shadow-x',field.shadowX||0],['shadow-y',field.shadowY||0],['outline',field.outlineWidth||0]].forEach(function(pair){var input=document.querySelector('#canvas-style-'+pair[0]);if(input)input.value=pair[1];});document.querySelector('#canvas-style-one-line').checked=!!field.oneLine;document.querySelector('#canvas-style-top').checked=!!field.noVerticalCenter;var family=document.querySelector('#canvas-style-family');family.replaceChildren();var none=document.createElement('option');none.value='';none.textContent='No family';family.appendChild(none);RulesTextStyles.options(key).forEach(function(item){var opt=document.createElement('option');opt.value=item.id;opt.textContent=item.name;family.appendChild(opt);if(item.selected)family.value=item.id;});family.disabled=!RulesTextStyles.options(key).length;}
	...
	function buildUI(){
		...
		document.querySelector('#canvas-style-create-family').onclick=function(){if(!editorArea||editorArea.kind!=='text')return;applyEditor();RulesTextStyles.createFamily(editorArea.key,false);refreshTextStyle(editorArea.key);};
